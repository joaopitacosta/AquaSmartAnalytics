DATA = dlmread('C:\TEMP\andrBream.csv',';');
% DATA = dlmread('C:\TEMP\andrBass.csv',';');

x = DATA(2:79,1); % temperature
y = DATA(1,2:36); % average weight
[xx,yy] = meshgrid(y,x);
% z = f(x,y)
z = DATA(2:79,2:36); % FCR value

fh1 = figure('Position',[0,0,900,675]);
surf(xx,yy,z,'FaceColor','interp');
title('Original Andromeda'); xlabel('Temp'); ylabel('Aver.wt.'); zlabel('FCR');
% shading interp;
colorbar;
...
title('Interpolated Andromeda'); xlabel('Temp'); ylabel('Aver.wt.'); zlabel('FCR');
% shading interp;
colorbar;
